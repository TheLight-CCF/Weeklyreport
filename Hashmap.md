...
+ 在JDK1.8之前 Hashmap 底层是 链表散列。
+ Hashmap 通过 key 的 hashCode 经过扰动函数处理过后得到 hash 值，然后通过 `(n - 1) & hash` 判断当前元素存放的位置，如果当前位置存在元素的话，就判断该元素与要存入的元素的 hash 值以及 key 是否相同，如果相同的话，直接覆盖，不相同就通过拉链法解决冲突。
...
+ 对于之前的版本 JDK1.8 以后在解决hash冲突时有了较大变化。
...
threshold = capacity * loadFactor**，**当 Size > threshold的时候，那么就要考虑对数组的扩增了，也就是说，这个的意思就是衡量数组是否需要扩增的一个标准。
...
Hashmap只提供了put用于添加元素，putVal方法只是给put方法调用的一个方法，用户并不能使用。

+ 关于 putVal 方法添加元素的算法：

  + 如果定位到的数组没有元素，就直接插入。
  + 如果定位到的数组有元素就和要插入的key比较，如果key相同就直接覆盖，如果key不相同，就判断p是否是一个树节点，如果是就调用`e = ((TreeNode<K,V>)p).putTreeVal(this, tab, hash, key, value)`将元素添加进入。如果不是就遍历链表插入(插入的是链表尾部)。

  ![putVal](D:\User\DoubleMoonBird7\文档\Note\Weeklyreport\images\putVal.png)
