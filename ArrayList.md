...
+ ArrayList的底层是数组队列，相当于动态数组吧。与普通的数组相比，它的容量是动态增长的。在添加大量元素前，应用程序可以使用 $ensureCapacity$ 操作来增加实际容量。可以减少递增式再分配的数量。
...
+ RandomAccess:这是一个标志接口，表明实现这个接口的 List 集合是支持快速随机访问的。在ArrayList中
+ Cloneable:表明它具有拷贝能力，可以进行深拷贝或浅拷贝操作。
+ Serializable:表明它可以进行序列化操作，也就是可以将对象转换为字节流进行持久化存储或网络传输，非常方便。

![ArrayList Interface and Class](D:\User\DoubleMoonBird7\文档\Note\Weeklyreport\images\ArrayList Interface and Class.png)

#### 它和Vector的区别

+ ArrayList是List的主要实现类，底层使用`Object[]`存储，适合频繁查找工作，线程不安全
+ Vector是List的古老实现类，底层使用`Object[]`存储，线程安全

#### 是否可以添加null值？

+ 可以，但不建议，null值没有意义而且会让代码难以维护，比如忘记做判空处理就会导致空指针异常。
...
+ 以上代码表明了，以无参数构造方法创建ArrayList时，实际上初始化赋值的是一个空数组。当真正对数组进行添加元素操作时，才真正分配容量。即向数组中添加第一个元素时，数组容量扩大至10。
...
+ `ensureCapacityInternal`方法内部直接调用了ensureExplicitCapacity
...
+ 当我们要`add`进第一个元素到`ArrayList`时，`elementData.length`为0（因为还是一个空的list），因为执行了`ensureCapacityInternal()`方法，所以`minCapacity`此时为10。现在，`minCapacity - elementData.length > 0`成立，随后执行`grow(minCapacity)`方法。
+ 当`add`第二个元素时，`minCapacity`为2，此时`elementData.length`（容量）在添加第一个元素后扩容成10了。现在`minCapacity - elementData.length > 0` 不成立，所以不会执行`grow(minCapacity)`方法。
+ 添加到3、4、……、10个元素时，依然不会执行 $grow$ 方法，数组容量都为10。

+ 直到添加到第11个元素，`minCapacity =  11 > elementData.length = 10`，随后进入到 $grow$ 方法进行扩容。 
...
`int newCapacity = oldCapacity + (oldCapacity >> 1);`，故每次扩容之后都会变成原来的1.5倍，但要主意`oldCapacity`的奇偶。

**hugeCapacity方法**

+ 当`newCapacity - MAX_ARRAY_SIZE > 0`条件成立时，就会执行`hugeCapacity`方法来比较`minCapacity`和`MAX_ARRAY_SIZE`，如果`minCapacity`大于最大容量，新容量为`Integer.MAX_VALUE`，否则，新容量大小则为 `MAX_ARRAY_SIZE;`
...
+ 看两者源代码会发现`copyOf()`内部调用了`System.arraycopy()`方法
...
`ArrayList` 源码中有一个 `ensureCapacity` 方法，这个方法 `ArrayList` 内部没有被调用过，所以肯定是给我们调用的，稍微写个例子测试以下有什么用：
